...
**归档与文件管理**
- 以主播为顶层归档：`{主播}/直播间/{日期}/`（直播录像）与 `{主播}/作品/`（订阅作品），与 NAS 同步副本同构
- 文件管理页按主播分组：每主播一张卡片显示文件数/体积/最后更新，展开分「直播间」「作品」两栏，保留平台筛选与播放/下载/合并/批量删除
- 主播改名不搬历史归档（目录名首落即固定）；旧布局文件在升级后首次启动自动迁移
...
- 录制文件落盘：`{output_dir}/{主播}/直播间/{日期}/{模板}.ts`，文件名模板支持 `{streamer} {room_id} {platform} {title} {remark} {date} {time} {datetime}` 占位符（设置页有实时预览）
...
- 作品文件落盘：`{output_dir}/{主播}/作品/{YYYYMMDD}_{标题}_{作品ID}.mp4`（图集为 `.zip`），与直播录像同属一个主播目录
...
```
recordings/                       # 以主播为顶层归档，直播与作品分栏
├── {主播}/直播间/{YYYY-MM-DD}/    # 直播录制（当日合并后为 {主播}_{日期}_合并.ts）
└── {主播}/作品/                   # 订阅作品（视频 mp4 / 图集 zip）
同步根目录（若启用）/               # NAS 备份
└── {主播}/直播|作品/...          # 按主播归拢的副本
...
│   │   ├── archive.py             # 归档目录布局单一来源 + 存量布局自动迁移
...
├── scripts/check_archive_layout.py # 归档布局自检：python scripts/check_archive_layout.py
├── Dockerfile / docker-compose.yml
└── README.md
```

**关于归档目录名**：目录名记在 `rooms.folder_name`，主播名下第一次落文件后即固定，之后在「主播管理」里改主播名/备注都不会搬迁历史归档（避免同一主播被拆成多个目录）。同名跨平台主播取 `主播名(平台)`；同平台多直播间（换号开播）共用一个目录。

**旧布局自动迁移**：升级后首次启动会把 `{平台}/{主播}/{日期}/` 与 `works/{平台}/{主播}/` 的存量文件逐个搬进上面的新结构，并回写录制记录/作品的相对路径。迁移逐文件校验目标位置确有文件才改库，搬不动的保留原位（列表仍识别旧路径），因此可反复启动重试、不会丢文件；主播已删除的孤儿目录按原名保留。
...
注意导出的设置里 Cookie / 代理 / Webhook 已脱敏（备份文件不含凭据），迁移后需在设置页重新粘贴。

---

## 🔒 安全与部署建议

- **务必部署在内网**，或放在已做鉴权的反向代理之后（QNAP 反向代理 Basic Auth、防火墙限来源 IP、或 Tailscale 只放自己设备，都是零代码改动的选择）。
- **敏感凭据不会外泄**：平台 Cookie、代理地址、Webhook 在所有 API 响应与「导出系统设置」里都以掩码返回（只给出字符长度）。设置页因此不回显旧值——留空=保持不变，粘贴新值=覆盖，单独输入 `-`=清除。反过来说，迁移到新机器后 Cookie 要重新粘一次（导出备份刻意不含凭据）。
- 环境变量 `LIVE_RECORDER_API_TOKEN` 可启用 Bearer Token 鉴权（`/api/*` 全部保护，健康检查与静态页豁免）。**注意**：开启后自带网页会不可用——前端的 `fetch`、`<video>` 播放与文件下载都不携带 `Authorization` 头，只有 `curl -H "Authorization: Bearer ..."` 这类客户端能用。要对外提供服务请放在反向代理后面，而不是打开这个变量。
- 未鉴权时 `/api/*` 对同网段完全开放，可增删主播、删文件、改设置，所以「只绑内网」这条不是可选项。
